...
<template>
  <div class="mx-auto max-w-sm px-4 py-10">
    <h1 class="text-xl font-bold text-orange-600">Create your Curb.ly account</h1>
    <form class="mt-6 flex flex-col gap-3" @submit.prevent="submit">
      <input v-model="name" type="text" placeholder="Name" class="rounded-lg border border-gray-300 px-3 py-2" required />
      ...
      <button type="submit" class="rounded-full bg-orange-600 py-2 text-sm font-medium text-white">Sign up</button>
    </form>
    <p class="mt-4 text-sm text-gray-600">
      Already have an account? <NuxtLink to="/login" class="text-orange-600 underline">Log in</NuxtLink>
    </p>
  </div>
</template>
